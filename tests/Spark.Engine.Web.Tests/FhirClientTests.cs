namespace Spark.Engine.Web.Tests
{
    ...
    public class FhirClientTests
    {
        ...
        {
            _server = new TestFhirServer(outputHelper, "https://localhost:6001");
        }

        [Theory]
        [InlineData(ResourceFormat.Json)]
        [InlineData(ResourceFormat.Xml)]
        public async Task CanCreatePatient(ResourceFormat format)
        {
            var client = new FhirClient("https://localhost:6001/fhir", new FhirClientSettings
            {
